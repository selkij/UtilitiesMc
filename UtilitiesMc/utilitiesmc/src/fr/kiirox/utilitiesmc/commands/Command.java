	...
	public boolean onCommand(CommandSender sender, org.bukkit.command.Command cmd, String msg, String[] args) {
    	if(sender instanceof Player) {
			...
			if(cmd.getName().equalsIgnoreCase("bc")) {
				//without argument
				if(args.length == 0) {
					player.sendMessage("§l§4Error: Bad use: §f/bc <message> or /alert <message>");
				}
				//with argument
				if(args.length >= 1) {
			...
			if(cmd.getName().equalsIgnoreCase("gm1")) {
				...
				if(args.length == 1) {
					...
					player.sendMessage("§l§4Error: The target is offline or does not exist!");
					} else {
				...
				if(args.length == 1) {
					...
					player.sendMessage("§l§4Error: The target is offline or does not exist!");
						...
						if(target.getGameMode() == GameMode.SURVIVAL) {
							...
							player.sendMessage("§l§6set [�4SURVIVAL�6] mode to §e"+ target.getName());
						}
					}
				}
			...
			if(cmd.getName().equalsIgnoreCase("fly")) {
				...
				if(args.length == 0) {
					if(player.getAllowFlight() == true) {
						...
						player.sendMessage("§l§6set [§4FLY§6] mode to §cOFF§6 for §e" + player.getName());
					} else {
						player.setAllowFlight(true);
						player.setFlying(true);
						player.sendMessage("§l§6set [§4FLY§6] mode to §2ON§6 for §e" + player.getName());
					}
				}
				//with arguments
				if(args.length == 1) {
					...
					if(target == null || !target.isOnline()) {
						...
						if(target.getAllowFlight() == true) {
							...
							player.sendMessage("§l§6set [§4FLY§6] mode to §cOFF§6 for §e" + target.getName());
							target.sendMessage("§l§6[§4FLY§6] mode has been set to §cOFF");
						} else {
							target.setAllowFlight(true);
							target.setFlying(true);
							player.sendMessage("§l§6set [§4FLY§6] mode to §2ON§6 for §e" + target.getName());
							target.sendMessage("§l§6[§4FLY§6] mode has been set to §2ON");
						}
					}
				}
